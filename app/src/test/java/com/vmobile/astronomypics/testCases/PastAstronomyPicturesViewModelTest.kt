...
class PastAstronomyPicturesViewModelTest {
    ...
    fun setup() {
        ...
        pastAstronomyPicturesViewModel = PastAstronomyPicturesViewModel(activity.application)
        pastAstronomyPicturesViewModel.updateDispatcher(coroutineTestRule.testDispatcher)
        ...
        addResponse(mockWebServer, mResources.openRawResource(R.raw.result_custom_date)
            .bufferedReader().use { it.readText() })
        planetaryRepository.updateNetworkService(apiService)
    }

    @Test
    fun `test custom date astronomy picture`() {
        runTest {
            pastAstronomyPicturesViewModel.getPictureOfDay("2022-11-03")
            Assert.assertEquals(pastAstronomyPicturesViewModel.pictureOfDay.value?.dateOfAPOD, "2022-11-03")
        }
    }
